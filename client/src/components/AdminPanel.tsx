        ...
        {projects?.length === 0 ? (
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {projects?.map((project) => (
              <div
                key={project.id}
                className="border border-gray-200 rounded-xl bg-white p-5 hover:border-orange-300 hover:shadow-md transition-all min-h-[160px]"
              >
                {editingProject?.id === project.id ? (
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      <div className={`w-12 h-12 ${getProjectColor(project.id)} rounded-xl flex items-center justify-center text-white font-bold text-lg flex-shrink-0`}>
