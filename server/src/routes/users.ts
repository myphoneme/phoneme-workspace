...
router.get('/workspace-sync', requireAdmin, async (_req: Request, res: Response): Promise<void> => {
  const metadata = await getWorkspaceSyncMetadata();
  res.json(metadata);
});
...
router.put('/:id', requireAdmin, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const existing = await db.prepare('SELECT * FROM users WHERE id = ?').get(userId) as User | undefined;
    if (!existing) {
      res.status(404).json({ error: 'User not found' });
      return;
    }
    ...
    if (email && email !== existing.email) {
      ...
      const emailExists = await db.prepare('SELECT id FROM users WHERE email = ? AND id != ?').get(email.toLowerCase(), userId);
      if (emailExists) {
        res.status(400).json({ error: 'Email already exists' });
        return;
      }
...
router.delete('/:id', requireAdmin, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const hasTodos = await db.prepare('SELECT id FROM todos WHERE assignerid = ? OR assigneeid = ?').get(userId, userId);
    if (hasTodos) {
